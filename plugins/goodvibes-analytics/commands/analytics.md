...
allowed-tools:
  - mcp__goodvibes-analytics__query
  - mcp__goodvibes-analytics__dashboard
  - mcp__goodvibes-analytics__budget
  - mcp__goodvibes-analytics__export
  - mcp__goodvibes-analytics__tag
  - mcp__goodvibes-analytics__sync
  - mcp__goodvibes-analytics__config
---
...
```
/goodvibes-analytics:analytics                    # Session summary (real data)
/goodvibes-analytics:analytics status              # Verbose session breakdown
/goodvibes-analytics:analytics mini                # Toggle the mini dashboard (tmux pane)
/goodvibes-analytics:analytics budget <amount>      # Set budget
/goodvibes-analytics:analytics budget               # Check budget
/goodvibes-analytics:analytics budget clear         # Clear budget
/goodvibes-analytics:analytics export [format]      # json | csv | markdown
/goodvibes-analytics:analytics tag add <name>       # Add a tag to the current session
/goodvibes-analytics:analytics tag remove <name>
/goodvibes-analytics:analytics tag list
/goodvibes-analytics:analytics tag auto             # Heuristic tag suggestions
/goodvibes-analytics:analytics sync                 # Sync current project into the global DB
/goodvibes-analytics:analytics sync all             # Sync every project under ~/.claude/projects/
/goodvibes-analytics:analytics config [key] [val]   # Get/set engine config
```
...
**Prerequisite check:** before calling any tool, verify the `mcp__goodvibes-analytics__*` tools
...
Call `mcp__goodvibes-analytics__query`:
```json
...
Call `mcp__goodvibes-analytics__dashboard`:
```json
...
Call `mcp__goodvibes-analytics__budget`:
```json
...
```
Invalid budget amount. Provide a positive number (e.g., /goodvibes-analytics:analytics budget 5.00)
```
...
Call `mcp__goodvibes-analytics__export`:
```json
...
Call `mcp__goodvibes-analytics__tag` with `action` set to `add`/`remove`/`list`/`auto` and
...
name:
```
Provide a tag name (e.g., /goodvibes-analytics:analytics tag add my-feature-work)
```
...
Call `mcp__goodvibes-analytics__sync` with `{ "scope": "current" }` or `{ "scope": "all" }`.
