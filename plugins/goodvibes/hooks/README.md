...
| `setup.mjs` | Setup (`init`) | intel | Runs once per project (marker in `.goodvibes/`), never touches the global home directory. Points at `/goodvibes:plugin setup` for native-dependency install — that command is the consent point, not this hook. |
