 ...
 */
export function rewritePlatformPaths(command: string): PathMappingResult {
  ...
  if (/\/dev\/stdin\b/.test(modifiedCommand)) {
    warnings.push(
      'WARNING: /dev/stdin detected - no direct Windows equivalent. ' +
  ...
  if (otherDevMatches) {
    warnings.push(
      'WARNING: Unix device paths detected (' + otherDevMatches.join(', ') + ') - ' +
