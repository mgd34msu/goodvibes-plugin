 ...
 */
export async function scanTodos(cwd: string): Promise<TodoScanResult> {
  const items: TodoItem[] = [];
  ...
  items.sort((a, b) => {
    ...
    const typeOrder: Record<string, number> = { BUG: 0, FIXME: 1, HACK: 2, TODO: 3, XXX: 4, NOTE: 5 };
 ...
 */
export function formatTodos(result: TodoScanResult): string | null {
