...
import { GatheredContext, formatContext, FormattedContext } from './formatter.js';
 ...
 */
export async function gatherProjectContext(cwd: string): Promise<ContextInjectionResult> {
  ...
  const formatted: FormattedContext = formatContext(gatheredContext);
 ...
 */
export async function gatherMinimalContext(cwd: string): Promise<{
  context: string;
  gatherTimeMs: number;
}> {
  const startTime = Date.now();
