...
export function parseRelativeTime(relativeStr: string): number {
...
export function resolveTimeFilter(filter?: TimeFilter): ParsedTimeFilter {
  const now = Date.now();
  if (!filter) {
...
export function isWithinTimeRange(timestamp: string | undefined, filter: ParsedTimeFilter): boolean {
