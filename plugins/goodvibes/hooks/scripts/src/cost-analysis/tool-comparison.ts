...
export function calculateToolMetrics(tools: ToolStats[]): ToolMetrics[] {
  return tools
    .filter(t => t.tool !== '__text_response__')
    .map(t => {
      const tk = t.tokens;
      const totalTokens =
        (tk.input || 0) + (tk.output || 0) + (tk.cacheRead || 0);
...
export function aggregateByCategory(
  ...
  const categoryMap = new Map<string, ToolMetrics[]>();
  ...
  for (const [category, tools] of categoryMap.entries()) {
    ...
    const avgInputPerCall =
      tools.reduce((sum, t) => sum + t.inputPerCall * t.calls, 0) / totalCalls;
    const avgOutputPerCall =
      tools.reduce((sum, t) => sum + t.outputPerCall * t.calls, 0) / totalCalls;
    const avgCachePerCall =
      tools.reduce((sum, t) => sum + t.cachePerCall * t.calls, 0) / totalCalls;
    const avgTotalPerCall =
      tools.reduce((sum, t) => sum + t.totalPerCall * t.calls, 0) / totalCalls;
    const avgCostPerCall =
      tools.reduce((sum, t) => sum + t.costPerCall * t.calls, 0) / totalCalls;
...
function calculateDelta(precision: number, native: number): number {
...
export function generateComparison(tools: ToolStats[]): ComparisonResult {
