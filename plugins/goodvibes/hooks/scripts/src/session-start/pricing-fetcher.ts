...
async function isCacheStale(ttlHours: number): Promise<boolean> {
  try {
...
async function fetchPricingMarkdown(url: string): Promise<string> {
...
function parseModelName(name: string): { family: 'opus' | 'sonnet' | 'haiku'; version: number } | null {
  ...
  const family = match[1].toLowerCase() as 'opus' | 'sonnet' | 'haiku';
  const version = parseFloat(match[2]);

  return { family, version };
}

function parsePricingTable(markdown: string): ModelFamily[] {
...
function filterLatestVersions(models: ModelFamily[]): ModelFamily[] {
  const latestByFamily = new Map<string, ModelFamily>();
...
function toCacheFormat(models: ModelFamily[]): PricingCache {
  const cacheModels: Record<string, ModelPricing> = {};
...
async function saveCache(cache: PricingCache): Promise<void> {
...
export async function fetchPricingIfStale(): Promise<void> {
  try {
