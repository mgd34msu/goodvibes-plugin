...
describe('recovery-patterns', () => {
  ...
  describe('TypeScript error patterns', () => {
    it('should include typescript_type_error with correct severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'typescript_type_error');
    ...
    it('should include typescript_config_error with correct severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'typescript_config_error');
      expect(pattern).toBeDefined();
    ...
    it('should match type assignment errors', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'typescript_type_error');
      expect(pattern?.patterns.some(p => p.test("Type 'string' is not assignable to type 'number'"))).toBe(true);
    });

    it('should match property not exist errors', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'typescript_type_error');
      expect(pattern?.patterns.some(p => p.test("Property 'foo' does not exist on type 'Bar'"))).toBe(true);
    });

    it('should match cannot find name errors', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'typescript_type_error');
      expect(pattern?.patterns.some(p => p.test("Cannot find name 'myVariable'"))).toBe(true);
    });

    it('should match argument not assignable errors', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'typescript_type_error');
      expect(pattern?.patterns.some(p => p.test("Argument of type 'string' is not assignable"))).toBe(true);
    });

    it('should match possibly undefined errors', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'typescript_type_error');
      expect(pattern?.patterns.some(p => p.test("Object is possibly 'undefined'"))).toBe(true);
    });

    it('should match possibly null errors', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'typescript_type_error');
      expect(pattern?.patterns.some(p => p.test("Object is possibly 'null'"))).toBe(true);
    });
  });

  describe('Import/Module error patterns', () => {
    it('should include missing_import with correct severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'missing_import');
      expect(pattern).toBeDefined();
      expect(pattern?.severity).toBe('high');
    ...
    it('should match cannot find module errors', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'missing_import');
      expect(pattern?.patterns.some(p => p.test("Cannot find module 'lodash'"))).toBe(true);
    });

    it('should match module not found errors', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'missing_import');
      expect(pattern?.patterns.some(p => p.test('Module not found: react'))).toBe(true);
    });

    it('should match unable to resolve path errors', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'missing_import');
      expect(pattern?.patterns.some(p => p.test('Unable to resolve path to module'))).toBe(true);
    });
  });

  describe('Type mismatch patterns', () => {
    it('should include type_mismatch with correct severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'type_mismatch');
      expect(pattern).toBeDefined();
      expect(pattern?.severity).toBe('medium');
    });
    ...
    it('should match expected arguments errors', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'type_mismatch');
      expect(pattern?.patterns.some(p => p.test('Expected 2 arguments, but got 3'))).toBe(true);
    });
  });

  describe('Runtime reference error patterns', () => {
    it('should include undefined_reference with correct severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'undefined_reference');
      expect(pattern).toBeDefined();
      expect(pattern?.severity).toBe('high');
    ...
    it('should match ReferenceError patterns', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'undefined_reference');
      expect(pattern?.patterns.some(p => p.test('ReferenceError: foo is not defined'))).toBe(true);
    });

    it('should match TypeError cannot read property patterns', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'undefined_reference');
      expect(pattern?.patterns.some(p => p.test('TypeError: Cannot read property of undefined'))).toBe(true);
      expect(pattern?.patterns.some(p => p.test('TypeError: Cannot read properties of null'))).toBe(true);
    });

    it('should match is not a function errors', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'undefined_reference');
      expect(pattern?.patterns.some(p => p.test('TypeError: foo is not a function'))).toBe(true);
    });
  });

  describe('Linting error patterns', () => {
    it('should include lint_error with low severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'lint_error');
      expect(pattern).toBeDefined();
    ...
    it('should match error count patterns', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'lint_error');
      expect(pattern?.patterns.some(p => p.test('5 errors and 3 warnings'))).toBe(true);
    });
  });

  describe('Test failure patterns', () => {
    it('should include test_failure with high severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'test_failure');
      expect(pattern).toBeDefined();
      expect(pattern?.severity).toBe('high');
    });
    ...
    it('should match FAIL patterns', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'test_failure');
      expect(pattern?.patterns.some(p => p.test('FAIL src/utils.test.ts'))).toBe(true);
    });

    it('should match AssertionError', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'test_failure');
      expect(pattern?.patterns.some(p => p.test('AssertionError: expected true'))).toBe(true);
    });

    it('should match vitest/jest/mocha', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'test_failure');
      expect(pattern?.patterns.some(p => p.test('vitest failed'))).toBe(true);
      expect(pattern?.patterns.some(p => p.test('jest test failure'))).toBe(true);
    });
  });

  describe('Build failure patterns', () => {
    it('should include build_failure with critical severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'build_failure');
      expect(pattern).toBeDefined();
    ...
    it('should match build failed patterns', () => {
      ...
      expect(pattern?.patterns.some(p => p.test('Compilation failed'))).toBe(true);
    });
  ...
  describe('NPM error patterns', () => {
    ...
    it('should match npm ERR! patterns', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'npm_error');
      expect(pattern?.patterns.some(p => p.test('npm ERR! code ERESOLVE'))).toBe(true);
    });

    it('should match ERESOLVE patterns', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'npm_error');
      expect(pattern?.patterns.some(p => p.test('ERESOLVE unable to resolve'))).toBe(true);
    });
  });

  describe('File system error patterns', () => {
    it('should include file_not_found with medium severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'file_not_found');
      expect(pattern).toBeDefined();
      expect(pattern?.severity).toBe('medium');
    });
    ...
    it('should match ENOENT patterns', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'file_not_found');
      expect(pattern?.patterns.some(p => p.test('ENOENT: no such file'))).toBe(true);
    });

    it('should include permission_error with high severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'permission_error');
      expect(pattern).toBeDefined();
      expect(pattern?.severity).toBe('high');
    });
    ...
    it('should match EACCES patterns', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'permission_error');
      expect(pattern?.patterns.some(p => p.test('EACCES: permission denied'))).toBe(true);
    });
  });
  ...
  describe('Git error patterns', () => {
    ...
    it('should match fatal: not a git repository', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'git_error');
      expect(pattern?.patterns.some(p => p.test('fatal: not a git repository'))).toBe(true);
    });

    it('should match failed to push', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'git_error');
      expect(pattern?.patterns.some(p => p.test('error: failed to push some refs'))).toBe(true);
    });

    it('should match merge conflicts', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'git_error');
      expect(pattern?.patterns.some(p => p.test('CONFLICT (content): Merge conflict in file.txt'))).toBe(true);
    });
  });

  describe('Database error patterns', () => {
    it('should include database_error with high severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'database_error');
      expect(pattern).toBeDefined();
      expect(pattern?.severity).toBe('high');
    ...
    it('should match ECONNREFUSED patterns', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'database_error');
      expect(pattern?.patterns.some(p => p.test('ECONNREFUSED 127.0.0.1:5432'))).toBe(true);
    });

    it('should match Prisma error codes', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'database_error');
      expect(pattern?.patterns.some(p => p.test('P2002: Unique constraint'))).toBe(true);
    });
  });
  ...
  describe('API/Network error patterns', () => {
    ...
    it('should match HTTP status codes', () => {
      ...
      expect(pattern?.patterns.some(p => p.test('status code 404'))).toBe(true);
    });
  });

  describe('Resource error patterns', () => {
    it('should include resource_error with critical severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'resource_error');
      expect(pattern).toBeDefined();
      expect(pattern?.severity).toBe('critical');
    ...
    it('should match heap out of memory', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'resource_error');
      expect(pattern?.patterns.some(p => p.test('JavaScript heap out of memory'))).toBe(true);
    });

    it('should match maximum call stack', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'resource_error');
      expect(pattern?.patterns.some(p => p.test('Maximum call stack size exceeded'))).toBe(true);
    });
  });

  describe('Syntax error patterns', () => {
    it('should include syntax_error with high severity', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'syntax_error');
      expect(pattern).toBeDefined();
      expect(pattern?.severity).toBe('high');
    });
    ...
    it('should match SyntaxError patterns', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'syntax_error');
      expect(pattern?.patterns.some(p => p.test('SyntaxError: Unexpected token'))).toBe(true);
    });

    it('should match unexpected token patterns', () => {
      const pattern = RECOVERY_PATTERNS.find(p => p.category === 'syntax_error');
      expect(pattern?.patterns.some(p => p.test('Unexpected token }'))).toBe(true);
    });
  });
