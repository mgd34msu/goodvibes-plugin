...
describe('error-patterns', () => {
  ...
  describe('pattern matching', () => {
    it('should match TypeScript error codes', () => {
      const tsPattern = RECOVERY_PATTERNS.find(p => p.category === 'typescript_type_error');
      expect(tsPattern?.patterns.some(p => p.test('TS2345: Error message'))).toBe(true);
    });

    it('should match type assignment errors', () => {
      const tsPattern = RECOVERY_PATTERNS.find(p => p.category === 'typescript_type_error');
      expect(tsPattern?.patterns.some(p => p.test("Type 'string' is not assignable to type 'number'"))).toBe(true);
    });

    it('should match property not exist errors', () => {
      const tsPattern = RECOVERY_PATTERNS.find(p => p.category === 'typescript_type_error');
      expect(tsPattern?.patterns.some(p => p.test("Property 'foo' does not exist on type 'Bar'"))).toBe(true);
    });

    it('should match module not found errors', () => {
      const importPattern = RECOVERY_PATTERNS.find(p => p.category === 'missing_import');
      expect(importPattern?.patterns.some(p => p.test("Cannot find module 'lodash'"))).toBe(true);
    });

    it('should match npm errors', () => {
      const npmPattern = RECOVERY_PATTERNS.find(p => p.category === 'npm_error');
      expect(npmPattern?.patterns.some(p => p.test('npm ERR! code ERESOLVE'))).toBe(true);
    });

    it('should match build failure errors', () => {
      const buildPattern = RECOVERY_PATTERNS.find(p => p.category === 'build_failure');
      expect(buildPattern?.patterns.some(p => p.test('Build failed'))).toBe(true);
    });

    it('should match test failure errors', () => {
      const testPattern = RECOVERY_PATTERNS.find(p => p.category === 'test_failure');
      expect(testPattern?.patterns.some(p => p.test('FAIL src/utils.test.ts'))).toBe(true);
    });

    it('should match git errors', () => {
      const gitPattern = RECOVERY_PATTERNS.find(p => p.category === 'git_error');
      expect(gitPattern?.patterns.some(p => p.test('fatal: not a git repository'))).toBe(true);
    });

    it('should match database connection errors', () => {
      const dbPattern = RECOVERY_PATTERNS.find(p => p.category === 'database_error');
      expect(dbPattern?.patterns.some(p => p.test('ECONNREFUSED 127.0.0.1:5432'))).toBe(true);
    });

    it('should match heap out of memory errors', () => {
      const resourcePattern = RECOVERY_PATTERNS.find(p => p.category === 'resource_error');
      expect(resourcePattern?.patterns.some(p => p.test('JavaScript heap out of memory'))).toBe(true);
    });

    it('should match syntax errors', () => {
      const syntaxPattern = RECOVERY_PATTERNS.find(p => p.category === 'syntax_error');
      expect(syntaxPattern?.patterns.some(p => p.test('SyntaxError: Unexpected token'))).toBe(true);
    });
  });
