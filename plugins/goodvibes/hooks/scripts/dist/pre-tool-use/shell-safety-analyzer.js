 ...
 */
export function isMcpPrecisionCall(command) {
 ...
 */
export function analyzeShellSafety(command) {
    const issues = [];
    if (!isMcpPrecisionCall(command)) {
        return { safe: true, issues: [] };
    }
    // Extract tool name
    const toolMatch = command.match(/plugin_goodvibes_precision-engine\/(precision_\w+|discover)/);
 ...
 */
export function attemptFix(command, issues) {
 ...
 */
export function formatBlockMessage(issues, toolName) {
