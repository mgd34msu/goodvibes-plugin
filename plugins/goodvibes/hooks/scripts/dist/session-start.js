...
var RuntimeClient = class _RuntimeClient {
  ...
  constructor(sessionId) {
    ...
    this.stateDir = join5(cwd, ".goodvibes", "state");
    ...
    try {
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          const pointerPath = join5(this.stateDir, entry);
          try {
  ...
  discoverSocket(sessionId) {
    ...
    if (sessionId && stateDirExists) {
      try {
        const sessionPointer = join5(stateDir, `runtime-${sessionId}.socket`);
      ...
      try {
        ...
        for (const entry of entries) {
          if (/^runtime-[a-zA-Z0-9_-]+\.socket$/.test(entry)) {
            try {
              const mtimeMs = statSync(join5(stateDir, entry)).mtimeMs;
        ...
        for (const { entry } of pointerFiles) {
          const pointerPath = join5(stateDir, entry);
          try {
