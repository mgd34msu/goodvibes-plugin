...
async function inspectCandidate(dir) {
  ...
  try {
    const content = await fs.promises.readFile(path2.join(dir, "CLAUDE.md"), "utf-8");
    importPresent = content.includes("<!-- GOODVIBES IMPORTS -->");
  } catch {
  }
  let goodvibesMdPresent = false;
  try {
    await fs.promises.access(path2.join(dir, ".goodvibes", "GOODVIBES.md"), fs.constants.R_OK);
    goodvibesMdPresent = true;
  } catch {
  }
  let promptDirPresent = false;
  try {
    const stat = await fs.promises.stat(path2.join(dir, ".goodvibes", "prompt"));
...
async function detectPromptInstallation(projectDir) {
  try {
    ...
    const claudeHome = path2.join(os.homedir(), ".claude");
    const resolvedProject = path2.resolve(projectDir);
    if (resolvedProject !== claudeHome && !resolvedProject.startsWith(claudeHome + path2.sep)) {
...
async function removeClaudeMdImports(projectDir) {
  const none = {
    removed: false,
    targetDir: null,
    importRemoved: false,
  ...
  try {
    ...
    try {
      const existing = await fs.promises.readFile(claudeMdPath, "utf-8");
      if (existing.includes("<!-- GOODVIBES IMPORTS -->")) {
        const updated = existing.replace(/[ \t]*<!-- GOODVIBES IMPORTS -->\s*\n@\.goodvibes\/GOODVIBES\.md[ \t]*\n?/g, "").replace(/\n{3,}/g, "\n\n");
    ...
    const promptDir = path2.join(targetDir, ".goodvibes", "prompt");
    for (const name of await installedPromptFileNames()) {
      const filePath = path2.join(promptDir, name);
      try {
    ...
    for (const dir of [promptDir, path2.join(targetDir, ".goodvibes")]) {
      try {
