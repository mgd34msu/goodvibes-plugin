...
export function formatGitContext(ctx) {
    ...
    // Changes
    const changesParts = [];
    if (ctx.stagedChanges > 0)
        changesParts.push(`${ctx.stagedChanges} staged`);
    if (ctx.unstagedChanges > 0)
        changesParts.push(`${ctx.unstagedChanges} unstaged`);
    if (ctx.untrackedFiles > 0)
        changesParts.push(`${ctx.untrackedFiles} untracked`);
