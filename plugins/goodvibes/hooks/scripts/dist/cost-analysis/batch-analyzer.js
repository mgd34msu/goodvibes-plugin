...
import { parseTimeFilter, walkDir, getProjectDirectories } from './parser.js';
...
export function extractBatchPayload(command) {
    try {
...
export function countBatchOperations(payload) {
    ...
    if (payload.operations) {
        for (const op of payload.operations) {
            ...
            if (tool.includes('read'))
                ...
                counts.edit++;
            else if (tool.includes('grep'))
                counts.grep++;
            else if (tool.includes('glob'))
                counts.glob++;
            else if (tool.includes('exec'))
                counts.exec++;
            else
                counts.other++;
        }
...
export function calculateBatchSavings(ops) {
    const nativeEquivalent = ops.read * NATIVE_READ_COST +
...
export function findBatchCalls(timeFilter) {
        ...
        for (const filePath of walkDir(dir)) {
            try {
                ...
                for (const line of lines) {
                    if (!line.includes('batch-engine/batch'))
                        continue;
                    try {
                        ...
                        if (entry.timestamp) {
                            ...
                            if (ts < filter.startTime || ts > filter.endTime)
                                continue;
                        }
...
export function analyzeBatches(timeFilter) {
