 ...
 */
export function getCostPer100Calls(tool) {
    // Check native tools
    if (tool in NATIVE_COSTS_PER_CALL) {
        return NATIVE_COSTS_PER_CALL[tool] * 100;
    }
    // Check MCP tools
    if (tool in MCP_COSTS_PER_CALL) {
        return MCP_COSTS_PER_CALL[tool] * 100;
    }
 ...
 * @throws Error if tool name is not recognized
 */
export function getOverheadAdjustedCostPer100(tool) {
