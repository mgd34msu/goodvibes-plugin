...
export function getProjectName(filePath) {
...
export function extractMcpTool(bashInput) {
    if (!bashInput || typeof bashInput !== 'object')
        return null;
...
export function parseTimeFilter(filter) {
    const now = Date.now();
    if (!filter) {
...
export function parseJournalFile(filePath, timeFilter, seenHashes) {
    const entries = [];
        ...
        for (const line of contentLines) {
            ...
            try {
                ...
                if (entry.timestamp) {
                    ...
                    if (timestamp < timeFilter.startTime || timestamp > timeFilter.endTime)
                        continue;
                }
    ...
    for (const [hash, data] of entryMap) {
        ...
        if (tools.length === 0)
            tools.push('__text_response__');
        entries.push({ model: data.model, usage: data.usage, tools });
    }
    return entries;
}
export function parseAllProjects(options = {}) {
    ...
    for (const file of files) {
        ...
        if (options.projectFilter && !options.projectFilter.includes(projectName))
            continue;
