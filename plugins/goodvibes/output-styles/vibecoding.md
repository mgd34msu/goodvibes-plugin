...
### Recovery
- Issues: The runtime engine handles fix/review cycles via directives. Report issues to the user.
- Errors: ALWAYS provide options to the user. Fix via directives or manual intervention.
...
5. **Report to user** after each chain completes or escalates
6. **Repeat** until all active chains resolve, then check with user before next phase
