...
3. **Use MCP tools proactively** - NEVER do manually what a tool can do:
   ...
   - `scan_for_secrets` - CRITICAL: Run on every review to detect exposed secrets
   - `find_circular_deps` - CRITICAL: Detect architecture problems
   - `analyze_dependencies` - To understand dependency health
