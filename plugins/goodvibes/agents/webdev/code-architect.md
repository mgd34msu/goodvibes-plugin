...
```bash
mcp-cli call plugin_goodvibes_goodvibes-tools/batch_read '{"files": ["src/index.ts", "src/utils/index.ts", "src/types/index.ts"], "output_mode": "minimal"}'
```
**Parameters:**
- `files` (required): Array of paths OR objects `{"path": "...", "offset": N, "limit": M}`
- `output_mode`: `"minimal"` | `"standard"` | `"verbose"`

### smart_glob
Find files by pattern for refactoring scope.
```bash
mcp-cli call plugin_goodvibes_goodvibes-tools/smart_glob '{"patterns": ["src/**/*.ts"], "exclude": ["**/*.test.ts", "**/*.d.ts"], "output_mode": "minimal"}'
```
**Parameters:**
...
### grep_with_content
Find usage patterns for refactoring.
```bash
mcp-cli call plugin_goodvibes_goodvibes-tools/grep_with_content '{"pattern": "import.*from.*oldModule", "glob": "**/*.ts", "output_mode": "minimal"}'
```
**Parameters:**
...
### atomic_multi_edit
Apply refactoring changes atomically.
```bash
mcp-cli call plugin_goodvibes_goodvibes-tools/atomic_multi_edit '{"edits": [...], "validation": {"run_typecheck": true}, "output_mode": "minimal"}'
```
**Parameters:**
- `edits` (required): Array of operations
- `validation`: `{"run_typecheck": true, "run_tests": true}`
- `dry_run`: Preview mode
- `output_mode`: `"count_only"` | `"minimal"` | `"standard"` | `"verbose"`

### workspace_symbols
Find symbols for rename/move operations.
```bash
mcp-cli call plugin_goodvibes_goodvibes-tools/workspace_symbols '{"query": "oldName", "kinds": ["function", "class", "type"], "output_mode": "minimal"}'
```
**Parameters:**
- `query` (required): Symbol name
- `kinds`: Symbol types to find
- `match_type`: `"exact"` | `"prefix"` | `"substring"`
- `output_mode`: `"count_only"` | `"minimal"` | `"standard"` | `"verbose"`

### get_document_symbols
Analyze file structure for splitting.
```bash
mcp-cli call plugin_goodvibes_goodvibes-tools/get_document_symbols '{"files": ["src/handlers/large-file.ts"], "output_mode": "verbose"}'
```
**Parameters:**
- `files`: Paths to analyze
- `output_mode`: `"count_only"` | `"minimal"` | `"standard"` | `"verbose"`
- `kind_filter`: Focus on specific symbol types
- `max_depth`: Nesting analysis depth
