...
var MODEL_PRICING_CACHE_PATH, FALLBACK_MODEL_PRICING, GLOBAL_CONFIG_PATH;
var init_config = __esm({
  "src/config.ts"() {
    "use strict";
    init_types();
    MODEL_PRICING_CACHE_PATH = join(homedir(), ".claude", "model-pricing.json");
...
var DEFAULT_MAX_SESSIONS, HistoricalStore;
  ...
  "src/data/historical-store.ts"() {
    ...
    HistoricalStore = class {
      ...
      list() {
        ...
        let files;
        try {
          files = readdirSync4(this.sessionsDir).filter((f) => f.endsWith(".json") && !f.startsWith(".tmp-"));
        } catch {
          return [];
        }
        const archives = [];
    ...
    JSONLScanner = class {
      ...
      expandTilde(inputPath) {
        if (inputPath.startsWith("~/") || inputPath === "~") {
          return path7.join(homedir6(), inputPath.slice(2));
        }
        return inputPath;
      }
    };
...
var Aggregator = class _Aggregator {
   ...
   */
  readStatuslineData() {
    const filePath = join9(homedir3(), ".claude", "debug-statusline-input.json");
    try {
