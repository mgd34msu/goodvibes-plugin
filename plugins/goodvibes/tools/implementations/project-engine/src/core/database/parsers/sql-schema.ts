 ...
 * SQL schema parser
 *
 * Parses raw SQL schema files (CREATE TABLE statements) into the
 * unified DatabaseSchemaResult format.
 *
 * @module core/database/parsers/sql-schema
 */
...
/**
 * Parse a raw SQL schema file into the unified DatabaseSchemaResult format.
 *
 * Supports:
 * - CREATE TABLE with IF NOT EXISTS
 * - Column definitions with PRIMARY KEY, NOT NULL, UNIQUE, REFERENCES
 ...
 */
export function parseSQLForUnifiedSchema(schemaPath: string): DatabaseSchemaResult {
    ...
    for (const line of lines) {
      ...
      if (/^(CONSTRAINT|PRIMARY|UNIQUE|CHECK|FOREIGN|INDEX|KEY)\b/i.test(normalizedLine)) {
        ...
        if (uniqueMatch) {
          ...
          const idxCols = uniqueMatch[3].split(',').map((c: string) => c.trim().replace(/[`"']/g, ''));
        ...
        if (idxMatch && !/UNIQUE/i.test(normalizedLine) && !/PRIMARY/i.test(normalizedLine)) {
          ...
          const idxCols = idxMatch[2].split(',').map((c: string) => c.trim().replace(/[`"']/g, ''));
  ...
  while ((match = createIndexRegex.exec(content)) !== null) {
    ...
    const idxCols = colsStr.split(',').map((c: string) => c.trim().replace(/[`"']/g, ''));
  ...
  return { source: 'sql', tables, relations, raw_path: schemaPath };
}
