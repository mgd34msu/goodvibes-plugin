...
import { readJsonFile, fileExists, safeExec, fetchUrl } from '../../shared/utils.js';
...
export async function handleUpgradePackage(
  ...
  try {
    ...
    if (!currentVersion) {
      return createErrorResponse(`Package "${packageName}" is not installed in this project`);
    }
    ...
    return createErrorResponse(`Failed to upgrade package: ${message}`);
  }
}
