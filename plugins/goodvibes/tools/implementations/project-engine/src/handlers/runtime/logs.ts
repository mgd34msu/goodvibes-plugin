...
export async function handleLogAnalyzer(
  ...
  if (args.source === 'file' && !args.path) {
    return createErrorResponse('path is required when source is "file"');
  }
  if (args.source === 'command' && !args.command) {
    return createErrorResponse('command is required when source is "command"');
  }
  ...
  if (nonEmptyLines.length === 0) {
    return createErrorResponse('No log entries found in source');
  }
