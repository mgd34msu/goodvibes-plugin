 ...
 *
 * @module handlers/runtime/memory
 */
...
export async function handleDetectMemoryLeaks(args: DetectMemoryLeaksArgs): Promise<ToolResponse> {
  ...
  if (target === 'pid' && (!inputPid || inputPid <= 0)) {
    return createErrorResponse('Invalid or missing PID for target type "pid"');
  }

  if (target === 'command' && !command) {
    return createErrorResponse('Missing command for target type "command"');
  }
  ...
  if (target === 'command') {
    try {
      ...
      if (!childProcess.pid || !isProcessAlive(childProcess.pid)) {
        return createErrorResponse(`Failed to start command: ${command}`);
      }

      monitoredPid = childProcess.pid;
      targetDescription = `command: ${command} (PID: ${monitoredPid})`;
    } catch (err) {
      return createErrorResponse(`Failed to spawn command: ${err instanceof Error ? err.message : String(err)}`);
    }
  } else {
    ...
    if (!isProcessAlive(monitoredPid)) {
      return createErrorResponse(`Process with PID ${monitoredPid} is not running`);
    }
  }
  ...
  if (snapshots.length < 2) {
    return createErrorResponse('Insufficient data collected. Process may have exited too quickly or memory data unavailable.');
  }
