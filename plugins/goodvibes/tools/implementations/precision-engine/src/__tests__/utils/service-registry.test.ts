...
describe('service-registry', () => {
  ...
  beforeEach(async () => {
    ...
    // Clear module caches to pick up new cwd
    vi.resetModules();
  ...
  describe('getFetchServices', () => {
    it('should return empty object when no services configured', () => {
      const services = registry.getFetchServices();
  ...
  describe('addService', () => {
    ...
    it('should throw on collision without force', async () => {
      ...
      await expect(
        registry.addService('test', { base_url: 'https://other.com' })
    ...
    it('should allow overwrite with force', async () => {
      ...
      const svc = registry.getService('test');
      expect(svc!.base_url).toBe('https://updated.com');
    });
  });
  ...
  describe('URL patterns', () => {
    ...
    it('should throw when adding pattern for nonexistent service', async () => {
      await expect(
        registry.addUrlPattern('api.example.com', 'nonexistent')
  ...
  describe('global defaults', () => {
    ...
    it('should set and retrieve global defaults', async () => {
      await registry.setFetchGlobalDefaults({
        headers: { 'User-Agent': 'PrecisionFetch/1.0' },
        timeout_ms: 5000,
      });
      const defaults = registry.getFetchGlobalDefaults();
      expect(defaults?.headers?.['User-Agent']).toBe('PrecisionFetch/1.0');
  ...
  describe('getServiceSummary / getAllServiceSummaries', () => {
    ...
    it('should return safe summary without secrets', async () => {
      ...
      // Should NOT include default_headers in summary
      expect((summary as any).default_headers).toBeUndefined();
    });
