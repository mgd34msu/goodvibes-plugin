/**
 * Tests for auth-orchestrator.ts
 */

import { describe, it, expect, vi, beforeEach, type Mock } from 'vitest';
import { applyAuth, handleAuthFailure, getAuthStatus, type AuthStatus } from '../../../utils/fetch/auth/auth-orchestrator.js';
import type { ServiceAuth } from '../../../utils/fetch/secrets-store.js';
import type { RequestAuth } from '../../../utils/fetch/request-builder.js';
import type { StoredCookie } from '../../../utils/fetch/cookie-jar.js';

// Mock dependencies
vi.mock('../../../utils/fetch/auth/static-auth.js', () => ({
  applyStaticAuth: vi.fn(),
}));

vi.mock('../../../utils/fetch/auth/oauth2-refresh.js', () => ({
  isTokenExpired: vi.fn(),
  canRefreshToken: vi.fn(),
  refreshAndStore: vi.fn(),
}));

vi.mock('../../../utils/fetch/auth/session-auth.js', () => ({
  canAcquireSession: vi.fn(),
  acquireAndStore: vi.fn(),
}));

vi.mock('../../../utils/fetch/secrets-store.js', () => ({
  getServiceSecrets: vi.fn(),
}));

vi.mock('../../../utils/fetch/cookie-jar.js', () => ({
  globalCookieJar: {
    getCookies: vi.fn(),
    toCookieHeader: vi.fn(),
  },
}));

import { applyStaticAuth } from '../../../utils/fetch/auth/static-auth.js';
import { isTokenExpired, canRefreshToken, refreshAndStore } from '../../../utils/fetch/auth/oauth2-refresh.js';
import { canAcquireSession, acquireAndStore } from '../../../utils/fetch/auth/session-auth.js';
import { getServiceSecrets } from '../../../utils/fetch/secrets-store.js';
import { globalCookieJar } from '../../../utils/fetch/cookie-jar.js';
...
describe('auth-orchestrator', () => {
  ...
  describe('applyAuth', () => {
    ...
    it('should apply per-request basic auth', async () => {
      ...
      const decoded = Buffer.from(headers['Authorization'].split(' ')[1], 'base64').toString('utf-8');
    ...
    it('should skip auth when per-request type is none', async () => {
      const headers: Record<string, string> = {};
      const requestAuth: RequestAuth = { type: 'none' };
    ...
    it('should refresh expired oauth2 token before applying', async () => {
      ...
      const refreshedAuth: ServiceAuth = {
        ...auth,
  ...
  describe('handleAuthFailure', () => {
    it('should return retry false for non-401 status', async () => {
      const result = await handleAuthFailure({ status: 403 }, 'test-service');

      expect(result).toEqual({ retry: false });
    });

    it('should return retry false when no service name provided', async () => {
      const result = await handleAuthFailure({ status: 401 });
    ...
    it('should acquire session token and return retry true', async () => {
      ...
      const acquiredAuth: ServiceAuth = {
        ...auth,
      ...
      (acquireAndStore as Mock).mockResolvedValue(acquiredAuth);
    ...
    it('should return needs_browser_auth hint for oauth2 without refresh', async () => {
      ...
      expect(result).toEqual({ retry: false, hint: 'needs_browser_auth' });
    });

    it('should return retry false for static auth failure', async () => {
      const auth: ServiceAuth = { type: 'bearer', token: 'bad-token' };
  ...
  describe('getAuthStatus', () => {
    ...
    it('should return no_credentials for bearer without token', async () => {
      const auth: ServiceAuth = { type: 'bearer' };
    ...
    it('should return no_credentials for oauth2 without access_token', async () => {
      const auth: ServiceAuth = { type: 'oauth2' };
    ...
    it('should return needs_refresh for expired oauth2 with refresh capability', async () => {
      ...
      expect(result).toBe('needs_refresh');
    });
    ...
    it('should return needs_browser_auth for expired oauth2 without refresh', async () => {
      ...
      expect(result).toBe('needs_browser_auth');
    });

    it('should return valid for static auth with credentials', async () => {
      const auth: ServiceAuth = { type: 'bearer', token: 'valid-token' };
