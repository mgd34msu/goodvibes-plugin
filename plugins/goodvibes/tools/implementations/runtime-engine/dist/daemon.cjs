...
var RuntimeEngineError, ConfigError, StateError, QueueError, ProcessingError, IPCError, WorkflowError, WorkflowTimeoutError;
...
function archiveFile(srcPath, archiveDir, errors) {
  try {
    ...
    const name = (0, import_node_path13.basename)(srcPath);
    const suffix = dateSuffix();
    const destPath = (0, import_node_path13.join)(archiveDir, `${name}.${suffix}.${Date.now()}`);
    (0, import_node_fs13.renameSync)(srcPath, destPath);
...
function performStateCleanup(opts) {
  ...
  const archivePointersDir = (0, import_node_path13.join)(stateDir, "archive", "pointers");
  const archiveSessionsDir = (0, import_node_path13.join)(stateDir, "archive", "sessions");
  ...
  for (const file of entries) {
    ...
    const filePath = (0, import_node_path13.join)(stateDir, file);
  ...
  for (const { dir, label } of archiveDirs) {
    ...
    let archiveEntries = [];
    try {
      archiveEntries = (0, import_node_fs13.readdirSync)(dir);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
    ...
    for (const file of archiveEntries) {
      const filePath = (0, import_node_path13.join)(dir, file);
      let mtimeMs = 0;
      try {
        mtimeMs = (0, import_node_fs13.statSync)(filePath).mtimeMs;
      } catch {
  ...
  if (activeSocketDir && (0, import_node_fs13.existsSync)(activeSocketDir)) {
    let sockFiles = [];
    try {
      sockFiles = (0, import_node_fs13.readdirSync)(activeSocketDir).filter((f) => f.endsWith(".sock"));
    } catch {
    }
    for (const sockFile of sockFiles) {
      ...
      if (!isPidAlive(pid)) {
        try {
          (0, import_node_fs13.unlinkSync)((0, import_node_path13.join)(activeSocketDir, sockFile));
...
var import_node_fs13, import_node_path13, log10, UUID_RE, PID_RE, SOCK_PID_RE;
...
var logger58, DEFAULT_DEVSERVER_CONFIG, DevServerMonitor;
var init_devserver = __esm({
  "src/plugins/devserver/index.ts"() {
    "use strict";
    init_logger();
    init_factories();
    logger58 = createLogger("devserver-monitor");
    ...
    DevServerMonitor = class {
      ...
      start() {
        if (!this.config.enabled) {
          logger58.debug("Dev server monitor disabled");
          return;
        }
        const interval = this.config.check_interval_ms ?? 15e3;
        this.timer = setInterval(() => void this.checkHealth(), interval);
        logger58.info("Dev server monitor started", { port: this.config.port, interval });
      }
      stop() {
        if (this.timer) {
          clearInterval(this.timer);
          this.timer = null;
          logger58.info("Dev server monitor stopped");
        }
      }
      async checkHealth() {
        ...
        try {
          ...
          if (response.ok) {
            if (!this.lastHealthy) {
              logger58.info("Dev server recovered", { port: this.config.port });
      ...
      emitError(error) {
        if (this.lastHealthy) {
          logger58.warn("Dev server unreachable", { port: this.config.port, error });
...
var JsonStateStore = class {
  ...
  async delete(key) {
    const path4 = this.keyPath(key);
    try {
      (0, import_node_fs7.unlinkSync)(path4);
...
var WorkflowPersistence = class {
  ...
  async persist(instance) {
    ...
    try {
      ...
      const finalPath = import_node_path8.default.join(this.stateDir, `${instance.id}.json`);
      await import_node_fs8.promises.writeFile(tmpPath, JSON.stringify(instance, null, 2), "utf-8");
      await import_node_fs8.promises.rename(tmpPath, finalPath);
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          log6.warn("Failed to restore workflow state file", { file });
        }
      }
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          const isTerminal = data["status"] === "completed" || data["status"] === "failed" || data["status"] === "cancelled" || data["status"] === "timed_out";
          if (isTerminal) {
            ...
            if (now - completedAt > this.ttlMs) {
              ...
              removed++;
              log6.debug("Removed expired workflow state file", { file });
            }
          }
        } catch {
        }
    ...
    if (webhookMatch !== null && method === "POST") {
      ...
      if (this.eventBus !== null && this.normalizers !== null) {
        try {
          ...
          this.eventBus.emit(event);
          logger42.info("webhook emitted directly to EventBus", { source, type: event.type });
          sendJson(res, 202, { accepted: true, source, type: event.type });
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
          logger42.error("failed to normalize/emit webhook", { source, error: msg });
...
var ExternalPlugin = class {
  ...
  setDirectEmit(eventBus) {
    if (this.listener !== null) {
      ...
      logger43.info("Direct EventBus emission enabled for HTTP listener");
    }
  }
  /**
...
var TickDriver = class _TickDriver {
  ...
  start() {
    ...
    if (this.isDaemonProcess) {
      ...
      logger44.info(`tick driver starting in ${mode} mode`, {
        eval_interval_ms: this.config.daemon.eval_interval_ms
      });
    }
    this.timer.start();
    ...
    if (this.isDaemonProcess && this.compactEventLog) {
      ...
      if (now - this.lastCompactionAt >= COMPACTION_INTERVAL_MS) {
        ...
        this.compactEventLog().catch((err) => {
          const msg = err instanceof Error ? err.message : String(err);
          logger44.warn("compactEventLog() error", { error: msg });
        });
      }
    }
  }
  ...
  sendToTmux(sessionName, content) {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logger44.warn("failed to send webhook content via tmux", { error: msg });
      return;
    }
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logger44.warn("failed to send first Enter via tmux", { error: msg });
      return;
    }
    this.sleepSync(TMUX_ENTER_DELAY_MS);
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logger44.warn("failed to send second Enter via tmux", { error: msg });
    }
  }
  /**
...
init_logger();
init_utils();
var logger46 = createLogger("daemon-tick-handler");
var DaemonTickHandler = class {
  ...
  async handleTick() {
    ...
    logger46.info("Daemon tick received", { tick_number: tickNumber });
    if (!this.budgetManager.canProcess()) {
      logger46.warn("Tick aborted: budget exceeded", { tick_number: tickNumber });
      return {
    ...
    logger46.info("Daemon tick completed", {
      tick_number: tickNumber,
      events_processed: eventsProcessed,
      duration_ms: Date.now() - startMs,
...
init_logger();
var logger47 = createLogger("action-executor");
    ...
    switch (action.type) {
      case "send_message": {
        ...
        if (typeof content !== "string" || content.length === 0) {
          logger47.error("ActionExecutor: send_message action missing content", {
            action_type: action.type,
        ...
        try {
          ...
          logger47.info("ActionExecutor: directive enqueued successfully", {
            target,
            priority,
            workflow_id: workflowId,
          ...
          if (this.agentWorkflowMap && params.agent_type && workflowId) {
            ...
            logger47.info("ActionExecutor: pending binds registered for spawn", {
              agent_type: agentType,
              workflow_id: workflowId
            });
          }
        } catch (err) {
          logger47.error("ActionExecutor: failed to enqueue directive", {
            target,
            priority,
            workflow_id: workflowId,
            error: err instanceof Error ? err.message : String(err)
      ...
      default: {
        logger47.warn("ActionExecutor: unhandled action type", {
          type: action.type,
          context
        });
        break;
...
init_logger();
init_utils();
var logger48 = createLogger("executor-subsystem");
function createExecutorSubsystem(config, eventBus) {
  try {
    ...
    logger48.info("Executor subsystem created", {
      mode,
      detection_method: executorMode.getDetectionMethod()
    });
    return { executorMode, executorBudget, daemonTickHandler };
  } catch (err) {
    logger48.warn("Executor subsystem creation failed \u2014 continuing without executor", {
      err: toErrorMessage(err)
    });
    return null;
  }
...
init_logger();
var logger49 = createLogger("adapter-registry");
...
init_logger();
var logger50 = createLogger("ipc-protocol");
...
function validateIPCMessage(obj) {
  ...
  switch (msg["type"]) {
    ...
    case "heartbeat":
      return true;
    default:
      logger50.warn("Unrecognised IPC message type", {
...
init_utils();
init_errors();
var logger51 = createLogger("ipc-server");
...
var IPCServer = class {
  ...
  async listen() {
    ...
    if ((0, import_node_fs12.existsSync)(this.socketPath)) {
      try {
        ...
        logger51.debug("Removed stale socket file", { path: this.socketPath });
      } catch (err) {
        logger51.warn("Could not remove stale socket file", {
    ...
    this.server.on("error", (err) => {
      logger51.error("IPC server error", { err: err.message });
    });
    return new Promise((resolve2, reject) => {
      ...
      srv.listen(this.socketPath, () => {
        ...
        logger51.info("IPC server listening", { path: this.socketPath });
   ...
   */
  async close() {
    logger51.info("Closing IPC server", {
      path: this.socketPath,
    ...
    return new Promise((resolve2) => {
      ...
      srv.close(() => {
        ...
        logger51.info("IPC server closed");
        resolve2();
      });
    });
  }
  ...
  handleConnection(socket) {
    ...
    if (this.isRateLimited()) {
      logger51.warn("IPC rate limit exceeded \u2014 rejecting connection", {
    ...
    this.recordMessage();
    logger51.debug("IPC client connected", { connections: this.connections.size });
    let idleTimer;
    socket.once("close", () => {
      clearTimeout(idleTimer);
      this.connections.delete(socket);
      logger51.debug("IPC client disconnected", { connections: this.connections.size });
    });
    socket.on("error", (err) => {
      clearTimeout(idleTimer);
      logger51.warn("IPC socket error", { err: err.message });
      const holdId = this.inFlightHolds.get(socket);
      if (holdId && this.writeResultCallback) {
        this.inFlightHolds.delete(socket);
        this.writeResultCallback(holdId, false);
    ...
    idleTimer = setTimeout(() => {
      logger51.warn("IPC connection timed out \u2014 closing", { timeout_ms: CONNECTION_TIMEOUT_MS });
      socket.destroy();
    ...
    socket.on("data", (chunk) => {
      ...
      if (rawBytes > MAX_MESSAGE_SIZE) {
        logger51.warn("IPC message size limit exceeded \u2014 closing connection", {
          size_bytes: rawBytes,
  ...
  processMessage(socket, line) {
    ...
    try {
      ...
      if (!validateIPCMessage(parsed)) {
        logger51.warn("IPC message failed schema validation \u2014 dropping", {
    ...
    if (!this.handler) {
      logger51.warn("No IPC message handler registered", { msg_type: message.type });
    ...
    logger51.debug("Dispatching IPC message", { id: message.id, type: message.type });
    this.handler(message).then((result) => {
      ...
      logger51.error("IPC handler threw an error", {
        id: message.id,
        err: toErrorMessage(err)
      });
      const errResponse = {
  ...
  writeResponse(socket, response, holdId) {
    ...
    try {
      ...
      logger51.warn("Failed to write IPC response", {
        id: response.id,
        err: toErrorMessage(err)
      });
      if (holdId && this.writeResultCallback) {
  ...
  removeSocketFile() {
    try {
      if ((0, import_node_fs12.existsSync)(this.socketPath)) {
        (0, import_node_fs12.unlinkSync)(this.socketPath);
        logger51.debug("Socket file removed", { path: this.socketPath });
      }
    } catch (err) {
      logger51.warn("Could not remove socket file", {
        path: this.socketPath,
        err: toErrorMessage(err)
      });
    }
...
var logger52 = createLogger("ipc-router");
var IPCRouter = class {
  static {
  ...
  removeSessionPointers() {
    ...
    for (const sessionId of this.registeredSessions) {
      ...
      try {
        ...
        logger52.debug("Session pointer file removed", { sessionId });
      } catch (err) {
        if (err.code !== "ENOENT") {
          logger52.warn("Could not remove session pointer file", {
            sessionId,
            err: toErrorMessage(err)
          });
        }
  ...
  async handleHookEvent(msg) {
    ...
    if (msg.hook_name === "session:started") {
      ...
      if (this.stateStore && typeof sessionId === "string" && sessionId.length > 0) {
        ...
        if (sessionKeys.length > 0) {
          logger52.info("Session cleanup: cleared stale WRFC state for session", {
            session_id: sessionId,
        ...
        if (defaultKeys.length > 0) {
          logger52.info("Session cleanup: cleared stale WRFC state from default namespace", {
      ...
      if (this.socketPath && this.stateDir && typeof sessionId === "string" && sessionId.length > 0) {
        try {
          ...
          logger52.info("Session pointer file written", { sessionId, pointer: pointerFile });
        } catch (err) {
          logger52.warn("Failed to write session pointer file", {
            sessionId,
            err: toErrorMessage(err)
          });
        }
      ...
      if (this.stateDir) {
        try {
          ...
          const result = performStateCleanup2({
            ...
            activeSocketDir: (0, import_node_path14.join)(this.stateDir, "sockets", "active")
          });
          if (result.archived > 0 || result.deleted > 0 || result.socketsRemoved > 0) {
            logger52.info("State cleanup", {
          ...
          if (result.errors.length > 0) {
            logger52.debug("State cleanup non-fatal errors", { errors: result.errors });
          }
        } catch (err) {
          logger52.warn("State cleanup failed", { error: toErrorMessage(err) });
        }
      }
    }
    if (msg.hook_name === "session:started" && this.socketPath && this.onSocketLostCallback) {
      if (!(0, import_node_fs14.existsSync)(this.socketPath)) {
        logger52.warn("Socket file missing at session start, triggering recreation", {
    ...
    if (msg.hook_name === "config:loaded" && this.directiveQueue) {
      ...
      if (wrfcConfig && typeof wrfcConfig === "object" && !Array.isArray(wrfcConfig)) {
        ...
        if (Object.keys(validated).length > 0) {
          ...
          logger52.debug("WRFC config stored from config:loaded event", { validated });
        }
      }
    }
    if (this.hookProcessor) {
      try {
        ...
        logger52.warn("IPC hook_event: HookProcessor error", {
  ...
  async handleQuery(msg) {
    ...
    logger52.warn("Unhandled query kind", { kind: q.kind });
    return { id: msg.id, status: "ok", data: { kind: "ack" } };
  }
  /**
  ...
  handleStateUpdate(msg) {
    logger52.debug("IPC state_update received (not implemented)", { id: msg.id });
   ...
   */
  async route(msg) {
    logger52.debug("IPC message received", { id: msg.id, type: msg.type });
...
init_logger();
var logger53 = createLogger("tool-gating");
var ToolGateEvaluator = class {
  ...
  evaluate(toolName) {
    try {
      ...
      for (const rule of this.config.rules) {
        ...
        if (fires) {
          logger53.debug("Tool blocked by rule", {
            tool: toolName,
      ...
      logger53.warn("Tool gate evaluation error \u2014 failing open", {
        tool: toolName,
        error: err instanceof Error ? err.message : String(err)
      });
      return { allow: true };
  ...
  evaluateCondition(rule) {
    switch (rule.condition) {
      ...
      case "custom":
        logger53.warn("Custom tool block condition is not yet implemented", {
          pattern: rule.tool_pattern
        });
        return false;
      default:
...
init_logger();
var logger54 = createLogger("context-injector");
  ...
  getContext() {
    ...
    for (const source of this.config.include) {
      try {
        ...
        logger54.warn("Context source failed \u2014 skipping", {
          source,
          error: err instanceof Error ? err.message : String(err)
        });
      }
...
var SocketWatcher = class {
  ...
  constructor(socketPath, onSocketLost, options) {
    this.socketPath = socketPath;
    this.socketDir = (0, import_node_path15.dirname)(socketPath);
    this.socketName = (0, import_node_path15.basename)(socketPath);
  ...
  start() {
    ...
    this.fired = false;
    try {
      const watcher = (0, import_node_fs15.watch)(this.socketDir, (eventType, filename) => {
...
function cleanStalePointerFiles(stateDir, log20) {
  try {
    let entries;
    try {
      entries = (0, import_node_fs16.readdirSync)(stateDir);
    } catch (err) {
      if (err.code === "ENOENT")
        return;
      throw err;
    ...
    for (const filename of pointerFiles) {
      ...
      if (isPidAlive(pid))
        continue;
      const pointerPath = (0, import_node_path16.join)(stateDir, filename);
      let socketFilePath;
      try {
        socketFilePath = (0, import_node_fs16.readFileSync)(pointerPath, "utf-8").trim();
      } catch {
      }
      let socketCleaned = false;
      if (socketFilePath) {
        try {
          (0, import_node_fs16.unlinkSync)(socketFilePath);
          socketCleaned = true;
        } catch (err) {
          if (err.code !== "ENOENT") {
      ...
      try {
        (0, import_node_fs16.unlinkSync)(pointerPath);
      } catch (err) {
        if (err.code !== "ENOENT") {
...
function removeSocketPointerFile(projectRoot, config) {
  const pointerFile = (0, import_node_path17.join)(
    projectRoot,
    config.persistence.state_dir,
    `runtime-${process.pid}.socket`
  );
  try {
    (0, import_node_fs17.unlinkSync)(pointerFile);
...
async function teardownIPC(subsystem, projectRoot, config) {
  ...
  if (subsystem.symlinkPath) {
    try {
      (0, import_node_fs17.unlinkSync)(subsystem.symlinkPath);
      logger56.debug("Symlink removed", { path: subsystem.symlinkPath });
    } catch (err) {
      if (err.code !== "ENOENT") {
        logger56.warn("Could not remove socket symlink", {
...
async function killProcessOnPort(port) {
  ...
  return new Promise((resolve2) => {
    let proc;
    if (os === "darwin") {
      proc = (0, import_node_child_process2.spawn)("sh", ["-c", `lsof -ti :${port} | xargs kill -9 2>/dev/null`], { stdio: "ignore" });
    } else {
      proc = (0, import_node_child_process2.spawn)("fuser", ["-k", `${port}/tcp`], { stdio: "ignore" });
    }
...
function restartDevServer(projectRoot) {
  return async (args, _event) => {
    ...
    try {
      ...
      const child = (0, import_node_child_process2.spawn)(cmd ?? command, cmdArgs, {
        cwd: projectRoot,
        detached: true,
        stdio: "ignore"
      });
...
function notifyUser(projectRoot) {
  return async (args, _event) => {
    ...
    const notificationsDir = (0, import_node_path18.join)(projectRoot, ".goodvibes", "notifications");
    const id = generateId();
    const ts = timestamp();
    const filename = `${ts}-${id}.json`;
    const filePath = (0, import_node_path18.join)(notificationsDir, filename);
    const record = {
      timestamp: (/* @__PURE__ */ new Date()).toISOString(),
      message,
      source,
    ...
    try {
      ...
      await (0, import_promises.writeFile)(filePath, JSON.stringify(record, null, 2), "utf-8");
...
async function notifyComplete(context, config) {
...
function logEvent(projectRoot) {
  return async (args, event) => {
    const logPath = (0, import_node_path19.join)(projectRoot, ".goodvibes", "logs", "activity.md");
    try {
      await (0, import_promises2.mkdir)((0, import_node_path19.dirname)(logPath), { recursive: true });
...
function updateMemory(projectRoot) {
  return async (args, _event) => {
    ...
    if (!key) {
      log15.warn("updateMemory called without args.key", { file });
      return;
    }
    const safeName = file.replace(/[^a-z0-9-]/gi, "");
    if (!ALLOWED_MEMORY_FILES.has(safeName)) {
      log15.warn("updateMemory blocked unknown memory file", { file, safeName });
      return;
    }
    const memoryDir = (0, import_node_path20.join)(projectRoot, ".goodvibes", "memory");
    const filePath = (0, import_node_path20.join)(memoryDir, `${safeName}.json`);
    try {
      await (0, import_promises3.mkdir)((0, import_node_path20.dirname)(filePath), { recursive: true });
...
function runBuild(projectRoot) {
  return async (context, config) => {
    const command = typeof config["command"] === "string" ? config["command"] : "npm run build";
    const cwd = typeof config["cwd"] === "string" ? config["cwd"] : projectRoot;
...
function runTests(projectRoot) {
  return async (context, config) => {
    const command = typeof config["command"] === "string" ? config["command"] : "npm test";
    const cwd = typeof config["cwd"] === "string" ? config["cwd"] : projectRoot;
...
function hasTestSuite(projectRoot) {
  return (_context, _event) => {
    const pkgPath = (0, import_node_path21.join)(projectRoot, "package.json");
    try {
      const raw = (0, import_node_fs18.readFileSync)(pkgPath, "utf-8");
...
init_logger();
init_events();
var logger57 = createLogger("build-test-detector");
...
var BuildTestDetector = class {
  constructor(eventBus, config = DEFAULT_DETECTOR_CONFIG) {
    this.eventBus = eventBus;
    this.config = config;
  }
  static {
  ...
  start() {
    ...
    logger57.info("BuildTestDetector started, listening for hook:post_tool_use");
  }
  analyzeToolResult(event) {
    ...
    this.eventBus.emit(emittedEvent);
    logger57.info("Detected command result", { category, succeeded, command, eventType });
  }
  isExecTool(toolName) {
...
// src/bootstrap.ts
var logger59 = createLogger("bootstrap");
...
var RuntimeEngine = class {
  ...
  // ─── Lifecycle ─────────────────────────────────────────────────────────────
  async startup() {
    logger59.info("Starting up");
    try {
      this.config = loadConfig(this.projectRoot);
      logger59.debug("Configuration loaded", { version: ENGINE_VERSION });
    } catch (err) {
      logger59.warn("Could not load config from disk \u2014 using defaults", { err: toErrorMessage(err) });
    }
    ...
    if (this.workflow) {
      ...
      const workflowPersistence = new WorkflowPersistence({
        stateDir: (0, import_node_path22.join)(this.projectRoot, ".goodvibes", "state", "workflows"),
        ttlMs: 24 * 60 * 60 * 1e3,
        // 24 hours
        enabled: true
      });
      ...
      for (const raw of savedInstances) {
        try {
          ...
          logger59.debug("Failed to restore workflow instance", { error: String(err) });
        }
      }
      if (savedInstances.length > 0) {
        logger59.debug("Restored workflow instances from persistence", { count: savedInstances.length });
      }
      ...
      this.events.eventBus.on("workflow:state_changed", (event) => {
        ...
        if (data?.instance?.id) {
          workflowPersistence.persist(data.instance).catch((err) => {
            logger59.warn("Failed to persist workflow state", { error: toErrorMessage(err) });
          });
        }
      });
    }
    this.events.eventBus.on("*", async (event) => {
      ...
      try {
        ...
        logger59.warn("Trigger evaluation error", { error: toErrorMessage(err) });
      }
    });
    ...
    if (this.workflow && this.directives) {
      this.watchdog = new WatchdogCoordinator({
        ...
        stateDir: (0, import_node_path22.join)(this.projectRoot, this.config.persistence.state_dir)
      });
    }
    ...
    const runtimeServices = {
      ...
      registerTrigger: (id, definition, handler) => {
        if (!coreTriggerRegistry) {
          logger59.warn("registerTrigger: trigger subsystem not available", { id });
          return;
        }
    ...
    logger59.debug("AgentTracker plugin registered", {
    ...
    if (this.triggers) {
      ...
      logger59.debug("Trigger action handlers registered");
    }
    ...
    if (wfEngine && this.coreRuntime) {
      ...
      logger59.debug("Workflow action handlers and guards registered");
    }
    const buildTestDetector = new BuildTestDetector(this.events.eventBus);
    buildTestDetector.start();
    logger59.debug("BuildTestDetector wired to EventBus");
    ...
    logger59.debug("Hook subsystem created", { handlerCount: hookSubsystem.hookRegistry.count() });
    ...
    if (httpEnabled && isDaemonProcess) {
      try {
        ...
        logger59.info("HTTP webhook listener started (direct EventBus emission)", {
          port: this.config.external.http_listener.port,
          host: this.config.external.http_listener.address
        });
      } catch (err) {
        logger59.warn("Failed to start HTTP webhook listener", { err: toErrorMessage(err) });
      }
    }
    ...
    if (this.config.devserver?.enabled) {
      ...
      logger59.debug("DevServerMonitor started");
    }
    if (!this.executorSubsystem?.executorMode) {
      logger59.warn("Skipping tick driver \u2014 executorMode not available");
    } else {
    ...
    if (this.config.features.ipc_enabled) {
        ...
        processHookEvent: async (event) => {
          ...
          if (processor) {
            try {
              ...
              logger59.warn("Failed to process hook event immediately", { error: toErrorMessage(err) });
            }
          }
        },
      ...
      logger59.debug("IPC server disabled by feature flag");
    }
    ...
    this.running = true;
    logger59.info("Startup complete", { pid: process.pid, uptime_ms: this.getUptime() });
  }
  async shutdown(timeout_ms = 1e4) {
    logger59.info("Shutting down", { timeout_ms });
    const shutdownTimer = setTimeout(() => {
      logger59.error("Shutdown timed out \u2014 forcing exit", { timeout_ms });
      process.exit(1);
    }, timeout_ms);
    shutdownTimer.unref();
    try {
      ...
      if (this.events) {
        try {
          ...
          logger59.warn("Failed to emit shutdown event", { err: toErrorMessage(err) });
        }
      }
      ...
      if (this.executorSubsystem?.executorBudget && coreStateStoreForShutdown) {
        try {
          ...
          logger59.warn("Executor budget persistence failed", { err: toErrorMessage(err) });
        }
      }
      if (this.persistence)
      ...
      this.running = false;
      logger59.info("Shutdown complete");
    } finally {
  ...
  updateConfig(config) {
    ...
    for (const [name, subsystem] of this.reconfigurables) {
      const sectionConfig = config[name];
      if (sectionConfig !== void 0 && typeof sectionConfig === "object" && sectionConfig !== null) {
        try {
          subsystem.reconfigure(sectionConfig);
        } catch (err) {
          logger59.warn(`Failed to reconfigure ${name}`, { error: toErrorMessage(err) });
        }
      }
    }
    if (this.externalPlugin) {
      this.reconfigureExternalPlugins(oldConfig, config).catch((err) => {
        logger59.error("Failed to reconfigure external plugins", { error: toErrorMessage(err) });
        this.events?.eventBus.emit({
          id: generateEventId(),
          timestamp: timestamp(),
          type: "system:error",
  ...
  async reconfigureExternalPlugins(oldConfig, newConfig) {
    ...
    if (wasEnabled && !nowEnabled) {
      try {
        ...
        logger59.info("HTTP webhook listener stopped (disabled by config change)");
      } catch (err) {
        logger59.warn("Failed to stop HTTP webhook listener", { error: toErrorMessage(err) });
      }
      ...
      try {
        ...
        await this.externalPlugin.startHttpListener();
        logger59.info("HTTP webhook listener started (enabled by config change)", {
          port: newHttp.port,
          host: newHttp.address
        });
      } catch (err) {
        logger59.error("Failed to start HTTP webhook listener after config change", {
          error: toErrorMessage(err),
          port: newHttp.port,
      ...
      try {
        ...
        this.externalPlugin.updateConfig(newExternalConfig);
        await this.externalPlugin.startHttpListener();
        logger59.info("HTTP webhook listener restarted (port change)", {
          oldPort: oldHttp.port,
          newPort: newHttp.port
        });
      } catch (err) {
        logger59.error("Failed to restart HTTP webhook listener on port change \u2014 attempting rollback", {
          error: toErrorMessage(err),
          oldPort: oldHttp.port,
          newPort: newHttp.port
        });
        try {
          ...
          await this.externalPlugin.startHttpListener();
          logger59.warn("HTTP webhook listener rolled back to previous port", { port: oldHttp.port });
        } catch (rollbackErr) {
          logger59.error("Rollback failed \u2014 HTTP webhook listener is permanently down", {
...
init_utils();
init_logger();
var logger60 = createLogger("daemon-server");
...
var DaemonServer = class {
  ...
  async start() {
    return new Promise((resolve2, reject) => {
      ...
      this.server.listen(this.socketPath, () => {
        ...
        this.server.on("error", (err) => logger60.error("Server error", { error: String(err) }));
        resolve2();
      });
    });
  }
  async stop() {
    ...
    this.sessions.clear();
    try {
      const { unlinkSync: unlinkSync11 } = await import("node:fs");
      unlinkSync11(this.socketPath);
    } catch {
    }
    if (!this.server)
      return;
  ...
  handleConnection(socket) {
    ...
    socket.on("error", (err) => {
      clearTimeout(idleTimer);
      logger60.error("Socket error", { error: String(err) });
    });
  }
  ...
  sendResponse(socket, resp) {
    try {
      ...
      logger60.error("Send error", { error: String(err) });
    }
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  async dispatchRPC(req) {
    ...
    switch (method) {
      ...
      case "cancelWorkflow": {
        ...
        if (!wf)
          throw new Error("WorkflowEngine not available");
        wf.cancel(args["workflowId"], args["reason"] ?? "cancelled via IPC");
        const cancelled = wf.get(args["workflowId"]);
...
init_utils();
init_fs_utils();
var logger61 = createLogger("daemon-hook-server");
var DaemonHookServer = class {
  ...
  async start() {
      ...
      processHookEvent: async (event) => {
        ...
        if (processor) {
          try {
            ...
            logger61.warn("processHookEvent failed", { error: toErrorMessage(err) });
          }
        }
      }
    });
    ...
    try {
      ...
      logger61.warn("Failed to write hook server pointer file", { err: toErrorMessage(err) });
    }
  }
  /**
  ...
  async stop() {
    ...
    if (this.ipcServer) {
      try {
        ...
        logger61.warn("Hook IPC server close error", { err: toErrorMessage(err) });
      }
      this.ipcServer = null;
    }
    this.ipcRouter = null;
    logger61.info("Daemon hook server stopped");
  }
  /**
  ...
  static socketPath(socketDir, projectRoot) {
    ...
    return (0, import_node_path23.join)(socketDir, `goodvibes-hook-${hash}-${process.pid}.sock`);
  }
};
...
// src/transport/daemon.ts
init_logger();
var logger62 = createLogger("daemon");
async function main() {
  const projectRoot = process.env["GV_PROJECT_ROOT"] ?? process.cwd();
  const goodvibesDir = (0, import_node_path24.resolve)(projectRoot, ".goodvibes");
  const socketPath = process.env["GV_DAEMON_SOCKET"] ? (0, import_node_path24.resolve)(process.env["GV_DAEMON_SOCKET"]) : (0, import_node_path24.resolve)(goodvibesDir, DAEMON_SOCKET_NAME);
  const hookSocketPath = process.env["GV_DAEMON_HOOK_SOCKET"] ? (0, import_node_path24.resolve)(process.env["GV_DAEMON_HOOK_SOCKET"]) : (0, import_node_path24.resolve)(goodvibesDir, DAEMON_HOOK_SOCKET_NAME);
  ...
  logger62.info("Daemon running", { pid: process.pid, socket: socketPath });
  const shutdown = /* @__PURE__ */ __name(async (signal) => {
    logger62.info("Received signal, shutting down", { signal });
    try {
