 ...
 */
async function safeExec(command, cwd, timeout = 30000) {
    try {
...
class GoodVibesServer {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                switch (name) {
                    ...
                    // Context gathering
                    case 'detect_stack':
                        return this.handleDetectStack(args);
                    case 'check_versions':
                        return this.handleCheckVersions(args);
                    case 'scan_patterns':
                        return this.handleScanPatterns(args);
                    // Live data
                    case 'fetch_docs':
    ...
    handleSkillDependencies(args) {
        ...
        if (this.skillsRegistry?.search_index) {
            for (const entry of this.skillsRegistry.search_index) {
                ...
                if (entryMeta.requires?.some(r => r.toLowerCase().includes(skill.name.toLowerCase()) ||
    ...
    async fetchNpmReadme(packageName) {
        try {
            const result = await safeExec(`npm view ${packageName} readme description repository.url homepage --json`, PROJECT_ROOT, 15000);
            if (result.error || !result.stdout) {
                return null;
            }
    ...
    getCommonApiReferences(library, topic) {
        ...
        if (topic) {
            ...
            const filtered = libraryRefs.filter(r => r.name.toLowerCase().includes(topicLower) ||
            ...
            if (runAll || checks.includes('naming')) {
                ...
                for (const match of funcMatches) {
                    ...
                    if (name.startsWith('_'))
                        continue; // Allow underscore prefix
        ...
        for (const filePath of attempts) {
            if (fs.existsSync(filePath)) {
                try {
                    ...
                    if (importsMatch) {
                        patterns.required_imports = importsMatch[1].match(/-\s*(.+)/g)?.map(m => m.replace(/^-\s*/, '').replace(/[`'"]/g, '').trim());
                    }
                    ...
                    if (avoidMatch) {
                        patterns.must_not_include = avoidMatch[1].match(/-\s*(.+)/g)?.map(m => m.replace(/^-\s*/, '').replace(/[`'"]/g, '').trim());
                    }
                    return patterns;
                }
                catch {
    ...
    async handleCheckTypes(args) {
        ...
        const result = await safeExec(`npx tsc --noEmit ${strictFlag} ${filesArg} 2>&1`, PROJECT_ROOT, 60000);
        const errors = [];
    ...
    async handleScaffoldProject(args) {
        ...
        const templateConfig = yaml.load(fs.readFileSync(templateYamlPath, 'utf-8'));
