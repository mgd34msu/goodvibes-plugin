...
export const VALIDATION_SCHEMAS = [
  ...
  {
    name: 'env_audit',
    description: 'Comprehensive environment variable audit. Scans source files for process.env.*, import.meta.env.*, Deno.env.* usages, compares .env against .env.example, identifies missing/unused/undocumented variables, and optionally validates value formats based on naming conventions (ports should be numbers, URLs should be valid, etc.).',
    inputSchema: {
      type: 'object',
      properties: {
        path: {
          type: 'string',
          description: 'Project root path to analyze (defaults to current directory)',
          default: '.',
        },
        env_file: {
          type: 'string',
        ...
        scan_code: {
          type: 'boolean',
          description: 'Scan source code for environment variable usages (default: true)',
          default: true,
        },
      },
    },
