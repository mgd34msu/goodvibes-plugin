 ...
 *
 * Extracts useEffect, useMemo, useCallback, useLayoutEffect, useInsertionEffect
 * calls from the AST and collects dependency array and body information.
 *
 * @module core/hooks/extractor
 */
 ...
 * Shared constant used by both extractor and issue-detector.
 */
...
/**
 * Check if a function node (callback) returns a cleanup function
 */
...
/**
 * Check if a function body text contains subscription/timer patterns
 */
...
/**
 * Extract the variable name from a hook's parent variable declaration
 * e.g., const myValue = useMemo(() => ..., [a]) → "myValue"
 */
  ...
  function visit(node: ts.Node): void {
    if (ts.isCallExpression(node)) {
      ...
      if (HOOKS_WITH_DEPS.has(fnName)) {
        ...
        const hookLine = line + 1; // convert to 1-indexed

        // Extract dependency array info
        let rawDeps: string[] = [];
        ...
        // Extract callback body text and references
        let body = '';
        let bodyRefs: string[] = [];
        let hasCleanup = false;
        ...
        // Register useMemo/useCallback variable names in scope for stability analysis
        if (variableName) {
        ...
        hooks.push({
          ...
          line: hookLine,
          variableName,
          deps: [], // filled by stability analyzer
          rawDeps,
          hasEmptyDeps,
          hasNoDeps,
          body,
...
/**
 * Build a ComponentScope by scanning the source file for imports,
 * module-scope declarations, useState/useReducer/useRef patterns.
 */
export function buildComponentScope(
  ...
  // Scan top-level statements for imports and module-scope declarations
  for (const statement of sourceFile.statements) {
    ...
    // Module-scope variable/function/class declarations
    if (
  ...
  // Scan component body for hook assignments (useState, useReducer, useRef, useId)
  function visitComponent(node: ts.Node): void {
    if (ts.isCallExpression(node)) {
      const fnText = node.expression.getText(sourceFile);
      const fnName = fnText.replace(/^React\./, '');

      // useState: const [val, setVal] = useState(...)
      if (fnName === 'useState') {
      ...
      // useCallback: const fn = useCallback(...)
      if (fnName === 'useCallback') {
        const parent = node.parent;
        if (ts.isVariableDeclaration(parent) && ts.isIdentifier(parent.name)) {
