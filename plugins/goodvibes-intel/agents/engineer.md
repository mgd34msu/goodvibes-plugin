...
Prefer `mcp__goodvibes-intel__*` for codebase search, reading, and static analysis
...
If the task involves a registered external service or credentialed API call, that's
`goodvibes-connect`'s `api_request`/`service` tools, not this server's job.
...
- **service-integration** (goodvibes-connect) — when wiring up a registered external service.
