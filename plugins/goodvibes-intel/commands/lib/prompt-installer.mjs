#!/usr/bin/env node
/**
 * Prompt-chain installer for `/goodvibes-intel:plugin install-prompts` /
 * `uninstall-prompts`.
 *
...
## Analytics
\`/goodvibes-analytics:analytics\` for session cost/token telemetry (a separate opt-in plugin).
...
---
*Installed by \`/goodvibes-intel:plugin install-prompts\`. Remove with \`uninstall-prompts\`.*
`;
