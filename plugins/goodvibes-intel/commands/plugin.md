---
description: goodvibes-intel plugin management — first-run native dependency setup (explicit consent), health status, and optional prompt-chain install
...
# GoodVibes Intel Plugin

Manage the goodvibes-intel plugin installation.

## Usage

```
/goodvibes-intel:plugin <subcommand>
```
...
The committed server bundle (`${CLAUDE_PLUGIN_ROOT}/server/index.cjs`) externalizes three
runtime-only dependencies listed in `${CLAUDE_PLUGIN_ROOT}/server/package.json`:
...
1. Check whether they're already installed:
   ```bash
   test -d "${CLAUDE_PLUGIN_ROOT}/server/node_modules/@ast-grep/napi" && test -d "${CLAUDE_PLUGIN_ROOT}/server/node_modules/@vscode/ripgrep" && echo ALREADY_INSTALLED || echo NEEDS_INSTALL
   ```
...
2. If `NEEDS_INSTALL`, tell the user exactly what will happen — `npm install` will run inside
   `${CLAUDE_PLUGIN_ROOT}/server/`, installing `@ast-grep/napi`, `@vscode/ripgrep`, and `sql.js`
   (native binaries + one WASM loader; no other package on the system is touched) — and confirm
   before proceeding.
3. On confirmation, run:
   ```bash
   npm install --omit=dev --no-audit --no-fund --prefix "${CLAUDE_PLUGIN_ROOT}/server"
   ```
4. Report success or the exact error output. On success:
   ```
   Native dependencies installed. goodvibes-intel's structure-aware search/analysis tools are ready.
   ```
...
2. Check `${CLAUDE_PLUGIN_ROOT}/server/index.cjs` exists (server bundle present).
...
Present as:
```
goodvibes-intel Status
=======================
Version: {version}
Server bundle: {present ? "Found" : "MISSING"}
Hooks registered: {event list, or "MISSING hooks.json"}
Native dependencies: {installed ? "Installed" : "Not installed — run /goodvibes-intel:plugin setup"}
