...
export class MiniRenderer {
  ...
  startLoop(
    ...
    // Draw immediately, then on interval. The draw loop holds no wake-lock —
    // the pane stays alive on its TTY and exits with the process via the
    // process-hygiene watchdog when the session is orphaned (field issue 9).
    draw();
