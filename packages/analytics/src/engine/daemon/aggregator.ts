...
/** Default logger: routed through core/logging (activity.log + stderr, never stdout). */
const DEFAULT_LOGGER: Logger = {
  warn: (msg) => engineLogger().warn(`[analytics:aggregator] ${msg}`),
};
...
export class Aggregator {
   ...
   */
  getActiveJsonlPath(): string | null {
  ...
  private scheduleGlobalDbSave(): void {
    ...
    // Debounced GlobalDB flush must never hold the loop open (field issue 9).
