 ...
 * `__tests__/utils/auth/oauth2-refresh.test.ts` (import paths only).
 */

import { describe, it, expect, vi } from 'vitest';
import { isTokenExpired, canRefreshToken, refreshAccessToken } from '../../fetch/auth/oauth2-refresh.js';
...
describe('oauth2-refresh', () => {
  ...
  describe('canRefreshToken', () => {
    it('should return true when all fields present', () => {
      expect(
        canRefreshToken({ type: 'oauth2', refresh_token: 'rt', token_url: 'https://auth.com/token', client_id: 'cid' }),
      ).toBe(true);
    });
    it('should return false when missing refresh_token', () => {
      expect(canRefreshToken({ type: 'oauth2', token_url: 'https://auth.com/token', client_id: 'cid' })).toBe(false);
    });
    it('should return false when missing token_url', () => {
      expect(canRefreshToken({ type: 'oauth2', refresh_token: 'rt', client_id: 'cid' })).toBe(false);
    });
  });
  ...
  describe('refreshAndStore', () => {
    it('should refresh and store updated auth', async () => {
      const refreshMod = await import('../../fetch/auth/oauth2-refresh.js');
      ...
      try {
        ...
        const result = await refreshMod.refreshAndStore('test-service', currentAuth);
        expect(result).not.toBeNull();
        expect(result?.access_token).toBe('new-token');
        expect(result?.refresh_token).toBe('new-refresh');
        expect(mockSetServiceSecret).toHaveBeenCalledWith(
          'test-service',
          expect.objectContaining({ access_token: 'new-token', refresh_token: 'new-refresh' }),
        );
      } finally {
        globalThis.fetch = originalFetch;
        vi.restoreAllMocks();
    ...
    it('should return null on refresh failure', async () => {
      const { refreshAndStore } = await import('../../fetch/auth/oauth2-refresh.js');
      const originalFetch = globalThis.fetch;
      globalThis.fetch = vi.fn().mockResolvedValue({
        ok: false,
        status: 401,
      ...
      try {
        ...
        expect(await refreshAndStore('test-service', currentAuth)).toBeNull();
      } finally {
        globalThis.fetch = originalFetch;
      }
    });
