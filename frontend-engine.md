...
#### `frontend_hook_dependencies` (new in v2)
...
#### `frontend_client_boundary` (new in v2)
...
#### `frontend_error_boundaries` (new in v2)
...
plugins/goodvibes/tools/
  ...
  implementations/frontend-engine/src/
    ...
    handlers/
      ...
      react.ts                          # Component tree builder (frontend_component_tree)
      jsx-class-utils.ts                # Shared: class extraction from JSX attributes
      response-utils.ts                 # Shared: response formatting

      trace-component-state.ts          # frontend_component_state entry
      analyze-render-triggers.ts        # frontend_render_triggers entry
      analyze-stacking-context.ts       # frontend_stacking_context entry
      analyze-tailwind-conflicts.ts     # frontend_tailwind_conflicts entry
      ...
      analyze-responsive-breakpoints.ts # frontend_responsive_breakpoints entry
      analyze-layout-hierarchy.ts       # frontend_layout_hierarchy entry
      ...
      get-sizing-strategy.ts            # frontend_sizing_strategy entry
      ...
      diagnose-overflow.ts              # frontend_overflow entry
      analyze-event-flow.ts             # frontend_event_flow entry
      ...
      get-accessibility-tree.ts         # frontend_accessibility_tree entry
      ...
      audit-hook-dependencies.ts        # frontend_hook_dependencies entry
      analyze-client-boundary.ts        # frontend_client_boundary entry
      analyze-error-boundaries.ts       # frontend_error_boundaries entry

      component-state/                  # frontend_component_state implementation
      ...
      render-triggers/                  # frontend_render_triggers implementation
      ...
      stacking-context/                 # frontend_stacking_context implementation
        context-rules.ts
        issue-detector.ts
        jsx-analyzer.ts
        portal-detector.ts
      ...
      responsive-breakpoints/           # frontend_responsive_breakpoints implementation
      ...
      overflow-diagnosis/               # frontend_overflow implementation
      ...
      hook-dependencies/                # frontend_hook_dependencies implementation (new in v2)
        hook-extractor.ts
        issue-detector.ts
        stability-analyzer.ts
        types.ts

      client-boundary/                  # frontend_client_boundary implementation (new in v2)
        graph-builder.ts
        issue-detector.ts
        scanner.ts
        types.ts

      error-boundaries/                 # frontend_error_boundaries implementation (new in v2)
...
| Tool | v2 Change |
|------|-----------|
| `frontend_component_tree` | Detects `memo`, `forwardRef`, `lazy`, and generic HOC wrapping; adds `wrappers` and `lazy` fields to output |
| `frontend_tailwind_conflicts` | Dynamic class extraction via shared `jsx-class-utils`: logical AND, clsx object syntax, array syntax |
| `frontend_responsive_breakpoints` | Custom breakpoint support via `breakpoint-resolver.ts`; auto-detects tailwind config; `breakpoints` parameter added |
...
| Tool | What It Does |
|------|--------------|
| `frontend_client_boundary` | Maps Next.js App Router server/client boundary; finds misclassified components and bundle bloat |
| `frontend_hook_dependencies` | Detects stale closures, missing/unnecessary deps, unstable references, and anti-patterns in hook dep arrays |
| `frontend_error_boundaries` | Maps error boundary coverage; finds unprotected subtrees and missing `error.tsx` in route segments |
...
- **Custom hooks** — `frontend_hook_dependencies` analyzes `useEffect`/`useMemo`/`useCallback`/`useLayoutEffect`/`useInsertionEffect` directly. Custom hooks that wrap these are not recursively analyzed.
